...
render函数返回jsx结构，用于描述具体的渲染内容

render函数应该是一个纯函数，不应该在里面产生副作用，比如setState。render函数每次渲染会被调用，而setState会触发重新渲染
...
该方法通过返回 true 或者 false 来确定是否需要触发新的渲染
...
getSnapshotBeforeUpdate 方法是配合 React 新的异步渲染的机制，在 DOM 更新发生前被调用
